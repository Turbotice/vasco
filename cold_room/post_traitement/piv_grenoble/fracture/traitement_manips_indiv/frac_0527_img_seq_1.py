...
bounds_indices2remove = (34,36) # upper bound included

xvals_cut = np.hstack((xvals[ind_inf_fit:bounds_indices2remove[0]],xvals[bounds_indices2remove[1]+1:ind_sup_fit]))
vvals_cut = np.hstack((v_converted_meters[frame_frac-i0,yind,ind_inf_fit:bounds_indices2remove[0]],v_converted_meters[frame_frac-i0,yind,bounds_indices2remove[1]+1:ind_sup_fit]))
print(xvals)
...
plt.plot(xvals_cut,vvals_cut,label='cleaned data, frame '+str(frame_frac))
