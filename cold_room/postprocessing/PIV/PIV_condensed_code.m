...
f_exc = 0.94;
freq_acq = 15;
disp(num2str(freq_acq,8));
optional_intermediate_dir = '';
acq_num = '3';
camera_SN = '22458101';
base = ['G:/Grenoble/' date '/manip_fracture/Acquisition_' acq_num '/camera_' camera_SN '/' num2str(f_exc,8) 'Hz_' num2str(freq_acq,8) 'Hz/'];

%base = ['/media/turbots/DATA/thiou/storageshared/Banquise/Vasco/Frigo_pmmh/' date '/' optional_intermediate_dir '/' num2str(f_exc,8) 'Hz_' num2str(freq_acq,8) 'Hz/'];
%base = ['/media/turbots/DATA/thiou/storageshared/Banquise/Vasco/Frigo_pmmh/' date '/' num2str(f_exc,8) 'Hz_' num2str(freq_acq,8) 'Hz/'];
%base = ['/media/turbots/DATA/thiou/storageshared/Banquise/Vasco/Frigo_pmmh/' date '/manip_f_k_membrane/Acquisition_1/camera_40300722/' num2str(f_exc,8) 'Hz_' num2str(freq_acq,8) 'Hz/'];
% 40232065
%40232066
%base = ['F:/manip_grenoble2024/fracture/20241125/Acquisition_4/camera_40307970/' num2str(f_exc,8) 'Hz_' num2str(freq_acq,8) 'Hz/'];
...
%     namesave= extractBetween([name '.'],'_offset','.');
    ...
    i0 = 4000; %process starting from image i0
    N = 5000; % last frame
    Dt = 1; %ratio between the fps and the scanning frequency (number of image between image A and image B)
...
%        [u,v,s,p] = PIVlab_commandline_movie_banquise(fullname,'',N,i0,Dt,b);
        ...
        save(matname,'u','v','s','p','xpix','ypix')
    else
        disp(matname)
...
load(fullname,'u','v','s','p','xpix','ypix');
